...
class LLMResponseDecider:
    ...
    def should_respond(
        ...
        prompt = f"""Ты — {bot_name}, участник живого группового чата.

Это близкий неформальный чат (друзья / семья). Твоя задача — после каждого сообщения решать, стоит ли влезать, и при этом чувствовать уместность.
...
В таком чате лучше быть смешным и дерзким, но не самым вульгарным и не самым ушедшим в абсурд. Если чувствуешь, что шутка уже на грани — лучше пропустить.

Примеры:

Сообщения:
user_a: сегодня на солане опять лонг поставил, +40%
user_b: вау, как ты это посчитал?
Ты должна ответить?
→ DECISION: YES
...
Сообщения:
user_a: батон что думаешь про этот рефакторинг?
Ты должна ответить?
→ DECISION: YES
REASON: прямо обратились ко мне

Сообщения:
user_a: в инете чекни погоду на завтра в казани
Ты должна ответить?
→ DECISION: YES
...
PATH:
- CASUAL — обычная болтовня, рофл, реакция на мем, короткий комментарий. Инструменты не нужны, можно ответить сразу.
...
При отсутствии явных слов "погугли / в инете / найди / прочитай / поставь" почти всегда CASUAL.
"""
        ...
        try:
            ...
            resp = client.create_message(
                ...
                system="Ты помогаешь решить, когда боту в живом групповом чате стоит ответить. Будь практичным: если человека явно просят что-то проверить или найти — это обычно хороший момент ответить. Не будь излишне осторожным.",
                model=self.model,
